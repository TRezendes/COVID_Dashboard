...
```{r}
#PA.counties.pop <- read_csv("/Users/trezendes/Downloads/co-est2020-alldata.csv") %>% filter(STNAME == "Pennsylvania") %>% select(CTYNAME, POPESTIMATE2020) %>% mutate_at("CTYNAME", str_replace, " County", "") %>% mutate_at("CTYNAME", str_replace, "McKean", "Mckean")

PA.counties.pop <- read_csv("C:\\Users\\timothy\\Dropbox\\SDS Default Folder\\R Projects\\COVID\\co-est2020-alldata.csv") %>% filter(STNAME == "Pennsylvania") %>% select(CTYNAME, POPESTIMATE2020) %>% mutate_at("CTYNAME", str_replace, " County", "") %>% mutate_at("CTYNAME", str_replace, "McKean", "Mckean")
...
ggplot() +
  ...
  coord_cartesian(xlim = c(as.POSIXct("2021-07-01"), as.POSIXct(Sys.Date())), ylim = c(0, 300))
...
```{r}
casesYrTransMog20.L <- covid.cases.latest.LANC %>% filter(date < as.POSIXct("2021-01-01")) %>% mutate(date = as.POSIXct(as.Date(date) + 731)) #%>% filter(date > as.POSIXct("2021-02-28"))

casesYrTransMog21.L <- covid.cases.latest.LANC %>% filter(date < as.POSIXct("2022-01-01")) %>% filter(date > as.POSIXct("2020-12-31")) %>% mutate(date = as.POSIXct(as.Date(date) + 365)) #%>% filter(date > as.POSIXct("2021-02-28"))
...
### Year Over Year - PA
```{r}
casesYrTransMog.P <- covid.cases.latest.PA %>% filter(date < as.POSIXct("2021-01-01")) %>% mutate(date = as.POSIXct(as.Date(date) + 366)) %>% filter(date > as.POSIXct("2021-02-28"))
#casesYrTransMog.P %>% arrange(date)

yMAX.P <- covid.cases.latest.PA %>% filter(date > as.POSIXct("2021-03-01")) %>% filter(cases < max(cases)) %>% select(cases) %>%  max()

ggplot() +
  geom_smooth(casesYrTransMog.P, mapping = aes(date, cases, color = "2020"), se = T) +
  geom_smooth(covid.cases.latest.PA, mapping = aes(date, cases, color = "2021"), se = T) +
  coord_cartesian(ylim = c(0, yMAX.P)) +
  scale_x_datetime(limits = c(as.POSIXct("2021-03-01"), as.POSIXct("2021-12-31"))) +
  scale_color_manual(name = "Year", breaks = c("2020", "2021"), values = c("blue", "red")) +
  geom_vline(xintercept = as.POSIXct("2021-05-25"), color= "light blue") +
  geom_vline(xintercept = as.POSIXct("2021-05-31"), color= "pink") +
  geom_vline(xintercept = as.POSIXct("2021-07-04"), color = "lavender") +
  geom_vline(xintercept = as.POSIXct("2021-09-07"), color= "light blue") +
  geom_vline(xintercept = as.POSIXct("2021-09-06"), color= "pink")
...
ggplot(covid.cases.latest.LANC) +
  ...
  coord_cartesian(c(as.POSIXct("2021-06-01"), as.POSIXct("2022-02-28")))
